...
def train_classifier(run_id, use_cuda, args):
    cfg = build_config(args.dataset, 
                       cv_train_views=getattr(args, 'cv_train_views', None),
                       cv_test_views=getattr(args, 'cv_test_views', None),
                       training_mode=getattr(args, 'training_mode', 'CS'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training_mode', type=str, default='standard',
                        choices=['standard', 'CS', 'CV'], help='Training mode')
    parser.add_argument('--cross_view_type', type=str, default='CS',
                        choices=['CS', 'CV'], help='Cross-view training type')
    ...
    parser.add_argument('--cv_train_views', type=str, default=None)
    parser.add_argument('--cv_test_views', type=str, default=None)
    
    parser.add_argument('--view_dropout_rate', type=float, default=0.0)
    
    parser.add_argument('--early_dropout_rate', type=float, default=0.0)

    parser.add_argument('--late_dropout_rate', type=float, default=0.1)

    parser.add_argument('--dropout_switch_epoch', type=int, default=10)
    
    parser.add_argument('--use_view_attention', action='store_true', default=True)
    
    parser.add_argument('--no_view_attention', dest='use_view_attention', action='store_false')
    
    parser.add_argument('--validation_mode', type=str, default='both', choices=['multi_view', 'single_view', 'both'])
    # parse arguments
    args = parser.parse_args()
    
    if args.cv_train_views:
        args.cv_train_views = [int(x.strip()) for x in args.cv_train_views.split(',')]
    if args.cv_test_views:
        args.cv_test_views = [int(x.strip()) for x in args.cv_test_views.split(',')]
